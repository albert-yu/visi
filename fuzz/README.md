...
**A fourth round then ran seventeen more unseen seeds** and stands at **3394 of 3400**, with every one of the six remaining attributed. It found §24, which replaced §16's 4×4 table with a single question asked once per side — *does the compiler know this operand's type?* — and dissolved the one cell §16 had written down as inexplicable: `(3# >= Empty)` is a compile-time constant whose type is `Variant`, so it is not a compile-time `Boolean`, and reading it as one is what forced the exception. Then §27, the same split on the one branch §23 had not reached, which only became reachable *because* of §24. Also one ordinary bug (§26, `IsNumeric(Empty)`), and a broader statement of §22's artifact (§25): an infinity produced by `^` makes Excel misreport *later, unrelated* faults as overflows, which is why one of its cases looked like a string-parsing bug and was not.

**Add seeds, and keep adding them.** Every one of those was found on a seed introduced *after* the previous round reported clean, and the corrected §16 was itself wrong on the first attempt in exactly the way the original was — then wrong again in §23, and again in §24. Two cases that cannot separate two rules look like confirmation of whichever you already believe, and the discriminating case has to be built on purpose: §24 needed a string where converting and comparing numerically give *opposite* answers, which none of the twenty-odd cases already on file did. A clean run on the seeds you already have says much less than one more seed does.
