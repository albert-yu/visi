...
**Phase 2 kept the pattern going.** On unseen seeds it stands at **998 of 1000**, and the cases it found on the way are the argument for the cell comparison in miniature: it overturned a Phase 1 rule that had been wrong since it was written (a runtime String against a static Boolean converts with `CBool`, it does not compare as text — §16 of the error-ordering doc), and it found two divergences **no return value could expose**, where the macro returned the same number on both sides and left the wrong thing in a cell. Assigning an infinity left `-inf` here and `#NUM!` in Excel; assigning `"  3  "` left text here and the number 3 in Excel.
