...
- **visi doesn't render filter/page fields as header rows -- confirmed real gap, not a harness bug.** Piloted against real Excel: Excel always shows a `FieldName | (All)` row plus a blank spacer row above the grid when a filter field exists; `libvisi/src/core/pivot.rs`'s `compute_pivot`/`PivotGrid` only ever uses `filter_fields` to filter records, never to add display rows (confirmed by reading the source -- `filter_fields` appears nowhere in the header/body row construction). This means **any pivot table with a filter field is completely cell-misaligned** between the two engines right now. This is a real gap in visi's own pivot rendering, out of scope for this harness to fix -- worth a dedicated follow-up in `pivot.rs`/`engine.rs`'s materialization code.
- **Unexplained wide-grid mismatches.** A handful of piloted configs show the two engines' grids spanning very different widths (mismatches appearing as far out as column AA). Not yet root-caused; candidates are a real visi bug, a bug in `BuildFuzzPivot.bas`, or the comparator diffing the whole sheet instead of just the pivot's destination rectangle. Needs further investigation before trusting broader iteration counts.
...
- **Layout alignment.** Excel's default new-pivot-table layout ("Compact Form") doesn't structurally match `visi`'s flat one-column-per-field tabular grid; `BuildFuzzPivot.bas` explicitly sets `LayoutForm = xlTabular`, `LayoutSubtotalLocation = xlAtBottom`, and `RepeatLabels = False` per row/col field to align the two.
