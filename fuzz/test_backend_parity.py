...
def test_run_returns_the_bytes_it_wrote(tmp_path):
    """[LLM-generated] The main loop parses the return value instead of re-reading the file."""
    src = str(tmp_path / "source.xlsx")
...
def test_empty_filter_selection_is_bindings_only(tmp_path):
    """[LLM-generated] `set_pivot_filter(name, col, [])` -- select nothing -- asserted here
...
def test_binding_error_text_matches_the_cli(tmp_path):
    """[LLM-generated] str(exc) must equal the CLI's stderr minus its "Error: " prefix."""
    import subprocess
...
def test_macro_add_parity(kind, sheet, tmp_path):
    """[LLM-generated] `visi macro add` and `Workbook.add_macro` must produce the same module.
...
def test_macro_list_parity(tmp_path):
    """[LLM-generated] The dicts `macros()` returns must carry `macro list --json`'s keys."""
    import json
    import subprocess
...
def test_macro_run_parity(tmp_path):
    """[LLM-generated] `visi macro run --output` and `Workbook.run_macro` + `save` must agree.
...
def test_macro_run_without_a_write_target_is_an_error_only_when_it_mutated(tmp_path):
    """[LLM-generated] The CLI refuses to discard a macro's writes; a read-only run is fine.
