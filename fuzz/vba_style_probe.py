...
def run_and_save(driver, xlsm, out_path):
    """[LLM-generated] Runs `Paint`, then has Excel save the workbook so openpyxl can read it."""
    script = "\n".join(
        [
...
def palette_channel(driver, workdir, batch):
    """[LLM-generated] What colour each of Excel's 56 `ColorIndex` slots actually is."""
