...
impl<T: CellType> Range<T> {
    ...
    /// ```
    ///
    /// PATCH (see PATCHES.md): `Range::new` panics if `start > end`, which
    /// upstream callers like `Xlsx::table_by_name` can reach with a table's
    /// data-only `Dimensions` when a table has a header row but zero data
    /// rows (the header-row adjustment pushes `start.0` past `end.0`).
    /// Guard against that here and return an empty range instead of
    /// panicking -- consistent with the "no overlap" case a few lines
    /// below, which already degrades gracefully rather than panicking.
