#!/usr/bin/env python3
"""[LLM-generated] Unit tests for the `visi_core` extension module.
...
# [LLM-generated] ------------------------------------------------------------ value mapping


@pytest.mark.parametrize(
    "src,expected,expected_type",
    [
        # [LLM-generated] A numeric *literal* stays an integer ...
        ("10", 10, int),
        # [LLM-generated] ... while arithmetic goes through floats, as Excel's model does.
        ("=1+1", 2.0, float),
        ("=1.5+1", 2.5, float),
...
def test_error_and_text_are_distinguished_by_type_not_equality():
    """[LLM-generated] The whole reason CellError exists.
    ...
    assert err == text  # [LLM-generated] equal as values ...
    assert type(err) is not type(text)  # [LLM-generated] ... but never the same thing
...
def test_get_display_renders_a_date():
    """[LLM-generated] A date is a serial number plus a format; only get_display renders it."""
    wb = visi_core.Workbook()
...
def test_exception_str_is_just_the_message():
    """[LLM-generated] `args` stays a 1-tuple so str(exc) matches the CLI's stderr text."""
    wb = visi_core.Workbook()
...
def test_unknown_enum_spelling_raises_invalid_argument():
    ...
    assert "column" in str(exc.value)  # [LLM-generated] lists what is accepted
...
def test_chart_id_changes_across_a_roundtrip():
    """[LLM-generated] Ids are re-derived on import, so a stale id must not be reused."""
    wb, name = _sheet_with_data()
    wb.add_chart(name, "column", f"{name}!A1:B3")
    again = wb.roundtrip()
    assert len(again.charts()) == 1
    # [LLM-generated] The point is not the specific value, it is that charts() is the only
...
def test_no_subtotal_is_applied():
    """[LLM-generated] Mirrors the CLI's post-add mutation; without it --no-subtotal is a no-op."""
    wb, _name, last = _pivot_source()
    wb.add_pivot_from_range(
        "P", start_row=0, start_col=0, end_row=last, end_col=2, dest_row=0, dest_col=5
    )
...
def test_subtotal_survives_a_roundtrip():
    """[LLM-generated] Contradicts pivot_xlsx.rs's stale module doc; the importer does read it."""
    wb, _name, last = _pivot_source()
    wb.add_pivot_from_range(
        "P", start_row=0, start_col=0, end_row=last, end_col=2, dest_row=0, dest_col=5
    )
...
def test_empty_filter_selection_is_expressible():
    """[LLM-generated] The state `visi pivot filter` cannot reach: select nothing.
...
def test_filter_selection_survives_a_roundtrip():
    """[LLM-generated] A selection is written as `<sharedItems>` indices and read back as values.
...
def test_a_filter_selecting_everything_reads_back_as_no_filter():
    """[LLM-generated] Nothing is marked hidden, so the file cannot tell the two apart.
...
def test_macro_survives_a_roundtrip():
    """[LLM-generated] The property the whole VBA feature rests on: the CLI is a fresh process
